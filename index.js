...
const sdk = require('postman-collection'),
    ...
    resolveUtils = require('./url-resolve-utils'),
...
function resolveNodeUrl (base, relative) {
    ...
    // if relative URL is empty, just remove hash from base and return
    if (!relative) {
        hashIndex = base.href.indexOf(SEARCH_SEPARATOR);
        ...
        return base.href.slice(0, hashIndex);
    }

    protocolEndIndex = PROTOCOL_PATTERN.exec(relative);

    if (protocolEndIndex) {
        protocolEndIndex = protocolEndIndex[0].length;
    }

    queryIndex = relative.indexOf(QUERY_SEPARATOR);
    hashIndex = relative.indexOf(SEARCH_SEPARATOR);
    ...
    // pathEndIndex = length of relative because there is nothing after path
    else {
        pathEndIndex = relative.length;
    }

    path = relative.slice(0, pathEndIndex);
    afterPath = relative.slice(pathEndIndex);

    // replace all '\' with '/' in path
    path = path.replace(/\\/g, PATH_SEPARATOR);

    // relative URL with all '\' replaced with '/' in path
    relative = path + afterPath;

    // pathStartIndex = index of first '/' after protocol 'http://'
    if (protocolEndIndex) {
        pathStartIndex = relative.indexOf('/', protocolEndIndex);
    }
    // path starts directly if relative URL doesn't have protocol
    else {
        pathStartIndex = 0;
    }

    // this means relative URL has protocol but it doesn't have path
    if (pathStartIndex === -1) {
        pathStartIndex = relative.length;
    }

    beforePath = relative.slice(0, pathStartIndex);
    path = relative.slice(pathStartIndex, pathEndIndex);
    afterPath = relative.slice(pathEndIndex);

    // if relative URL has protocol, traverse path and return it as it is.
    if (PROTOCOL_PATTERN.exec(relative)) {
        return beforePath + resolveUtils.traversePath(path) + afterPath;
    }

    // for protocol-relative URL, just add protocol from base and return
    // don't traverse path for it
    if (relative.startsWith(DOUBLE_SLASH)) {
        return base.protocol + relative;
    }

    // relative URL is root-relative URL
    if (relative.startsWith(PATH_SEPARATOR)) {
        return resolveUtils.getRootUrl(base) + resolveUtils.traversePath(path) + afterPath;
    }

    // relative URL is base-relative URL
    path = resolveUtils.getBasePath(base) + path;
